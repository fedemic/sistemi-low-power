...
Al fine di ottenere una stima più accurata del consumo di potenza del circuito preso in considerazione, è necessario avere valutazione più realistica dell'attività dei singoli nodi del circuito sintetizzato. A tale scopo è possibile effettuare una simulazione back-annotated, generando post-sintetizzazione un file contenente la descrizione della netlist del circuito.
\\
...
È possibile applicare una serie di soluzioni architetturali  al fine di ridurre il consumo di potenza dinamica di un generico circuito che presenta un datapath delimitato da barriere di registri.
\\
...
\\\\
\centerline{percorso critico = reg + inc + comp + mux = 140 ns\\}
\centerline{frequenza massima = 7.14MHz\\}
\centerline{potenza @ 5MHz = 3 reg + 2 inc + comp + mux = 10.73 uW\\}
\centerline{area = 3 reg + 2 inc + comp + mux = 1747 um 2\\}
\\\\
...
\centerline{\includegraphics[width=10cm]{./img/Lab_3/clock_gating_implementation.png}}
\\\\
In particolare, raddoppiando il datapath e inserendo un opportuno multiplexer, è possibile far lavorare i componenti a metà della frequenza originale, mantenendo comunque lo stesso throughput in uscita del circuito originale. Data l'espressione per valutare il consumo di potenza dinamica:
\\\\
\centerline{P = Cf...}
\\\\
...
\\\\
Dal momento che la tensione di alimentazione contribuisce in modo quadratico al consumo di potenza, quest'ultima risulterà ridotta di un fattore u 2, dove u rappresenta il fattore di riduzione di Vdd.
\\\\
\centerline{P new = P nom * u 2}
\\\\
...
\\\\
\centerline{T new = T nom * 0.75u/u-0.25}
\\\\
...
\begin{center}
	\begin{tabular}{|c|c|c|c|}
	\hline
	Cella & Ritardo [ns] & Potenza [uW] & Area [um 2] \\
	\hline
	 registro & 3 & 0.15 & 319 \\
	\hline
	 incrementatore & 60 & 0.6375 & 256 \\
	\hline
	 comparatore & 126 & 0.54 & 161 \\
	\hline
	 mux & 21 & 0.4175 & 117 \\
	\hline
	\end{tabular}	
...
\\\\
\centerline{percorso critico = reg + inc + comp + mux + mux = 231 ns\\}
\centerline{frequenza massima = 4.3 MHz\\}
\centerline{potenza @ 2.5 MHz = 3 reg + 2 inc + comp + 2 mux = 3.1 uW\\}
\centerline{area = 2*(3 reg + 2 inc + comp + mux) + mux = 3611 um 2\\}
\\\\
Si può notare che la potenza è stata ridotta di un fattore 3.46 a fronte di un raddoppio dell'area. In particolare, è possbile osservare l'effetto dell'overhead del mux in uscita nella valutazione del percorso critico, che risulta maggiore.
\\\\
Un'altra tecnica a livello architetturali applicabile in questo contesto è la pipeline. L'inserimento di registri all'interno del percorso combinatorio permette di ridurre il percorso critico, mantenendo comunque un throughput elevato.
\\
Riducendo il percorso critico si potrebbe lavorare ad una frequenza maggiore rispetto alla frequenza nominale, tuttavia si decide di mantenere la stessa frequenza del circuito originale, avendo quindi la possibilità di incrementare i ritardi attraverso una riduzione di Vdd.
\\
...
\\\\
\centerline{11.63/5 = 2.33}
\\\\
A tale fattore di scalamento corrisponde una u max = 0.37, ricavato invertendo la formula precedentemente riportata. In questo caso, è stata ricercata la massima riduzione possibile della tensione di alimentazione.
\\\\
\begin{center}
	\begin{tabular}{|c|c|c|c|}
	\hline
...
\end{center}
\vspace{0.3cm}
\centerline{percorso critico = reg +  comp = 198.85 ns\\}
\centerline{frequenza massima = 5.03 MHz\\}
\centerline{potenza @ 5 MHz = 3 reg + 2 inc + comp + mux + 3 reg (pipe) = 1.7 uW\\}
\centerline{area = 3 reg + 2 inc + comp + mux + 3reg (pipe) = 2704 um 2\\}
\vspace{0.3cm}
Come atteso, si nota una significativa riduzione del consumo di potenza, pari ad un fattore 6.3. Inoltre, il circuito risulta più compatto rispetto al caso della parallelizzazione.
\\
In generale, la tecnica della pipeline è permette di raggiungere risultati migliori in termini di trade-off tra potenza e area. Tuttavia tale soluzione modifica radicalmente il timing del sistema, il quale rimane invariato nel caso di utilizzo della tecnica di parallelizzazione.
\\
...
\\\\
Per confronto si è cercato il minimo consumo di potenza applicando una solamente la tecnica della parallelizzazione, con parallelismo 2.  In seguito alla riduzione di Vdd, il minimo consumo di potenza dinamica ottenibile è pari a 1.98 uW, scegliendo u = 0.4.
\\
Per raggiungere le stesse prestazioni della pipeline in termini di riduzione dei consumi, è necessario introdurre un ulteriore livello di parallelismo: in questo caso è possibile selezionare u = 0.37, ottenendo quindi un consumo di potenza pari a 1.74 uW, comparabile con il risultato ottenuto con la pipeline.
\\\\
Partendo dalla soluzione con pipeline, sono stati ricercati possibili miglioramenti unendo le due tecniche. Il grafico seguente mostra il prodotto potenza ritardo in funzione del fattore u, considerando la presenza di pipeline e di un parallelismo pari a 2.
\\\\
\centerline{\includegraphics[width=15cm]{./img/Lab_3/prodotto_potenza_ritardo.png}}
\\\\
...
\\
Si fa presente che l'analisi effettuata è relativa alla sola potenza dinamica, infatti una riduzione significativa di Vdd comporta l'utilizzo di transistor caratterizzati da una tensione di soglia minore, aumentando il contributo di corrente di leakage.
...
\\
Considerando le forme d'onda in ingresso come mostrato nella figura seguente, è possibile stimare l'andamento dell'uscita C.
\textcolor{red}{TIMING}
\\\\
\centerline{\includegraphics[width=15cm]{./img/Lab_3/prodotto_potenza_ritardo.png}}
\\\\
...
\textcolor{red}{
\begin{itemize}
\item schema clock gating
\item datapath parallelizzazione
\item datapath pipeline
\item forme d'onda
\end{itemize}}
