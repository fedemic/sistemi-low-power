...
In particolare, in tale sezione è trattata la simulazione di un full adder descritto in modi differenti in tre file, \textbf{rtl\_adder.sv}, \textbf{gate\_adder.sv} e \textbf{gate2\_adder.sv}.\\
All'interno di file .f sono inserite le direttive per compilare i file di descrizione necessari e le macro per scegliere tra diverse implementazioni dello stesso circuito.\\\\
...
Ciò è verificabile anche attraverso la rappresentazione delle forme d'onda dei segnali coinvolti, utilizzando il file \textbf{run\_rtl.do}.\\
\\\\
...
\\\\
È inoltre possibile utilizzare testbench in System Verilog per simulare descrizioni di circuiti in VHDL. Per fare ciò è necessario aggiungere all'interno del file \textbf{adder\_test.sv} un'opportuna direttiva avente il fine di includere la possibilità di testare l'istanza descritta in VHDL. 
...
\\
Il codice sviluppato è riportato in appendice A.
\\\\
