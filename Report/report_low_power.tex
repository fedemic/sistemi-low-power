...
\chapter{Power Estimation: probabilistic techniques}
...
\newpage
\chapter{FSM Assignment and VHDL Synthesis}    
...
L'obiettivo di questa prima parte consiste nella caratterizzazione dell'andamento delle correnti all'interno di una porta NAND in logica CMOS, la cui descrizione è contenuta allinterno del file \textbf{CMOS013.spi}.
I transistor che definiscono la netlist del gate sono prelevati da una libreria contenente modelli di MOS high speed e low leakage. Tali modelli sono particolarmente complessi dal momento che prendono in considerazione un elevato numero di parametri che influenzano il comportamento del dispositivo.
\\\\
In particolare, la netlist descrive il seguente circuito.
\textcolor{red}{IMMAGINE NETLIST}
\\\\
Le direttive di simulazione sono presenti all'interno del file \textbf{nandHS.sp}, all'interno del quale è istanziato il circuito interessato. Sono definiti alimentazione, massa, carico e ingressi del circuito; di questi uno è costante, l'altro è definito da una serie di transizioni.
\\
...
\\\\
In seguito è stata effettuata un'analisi delle forme d'onda attraverso il comando:
\\\\
...
\\\\
All'interno di un modello di un transistor è tenuto conto della variazione della tensione di soglia rispetto alla differenza di potenziale tra source e bulk (effetto body) e tra source e drain (DIBL). In particolare, tali tensioni sono differenti per transistor disposti in serie a causa della ripartizione di tensione tra questi.
\\\\
...
\end{center}
\vspace{0.3cm}
Si nota come all'aumentare della capacità di carico si abbia un incremento dei parametri temporali. Come conseguenza si ha infatti un progressivo incremento delle costanti di tempo per le varie transizioni. 
\\\\
Successivamente ad un fronte di salita sull'uscita, è possibile che non solo è presente un contributo di corrente che scorre dall'alimentazione verso il carico, ma anche un contributo verso massa. Tale comportamento è inatteso secondo il comportamento ideale dei transistori. In realtà, in seguito ad una commutazione, vi è un breve intervallo di tempo durante il quale entrambi i dispositivi sono in conduzione, generando un contributo di corrente di corto circuito tra alimentazione e massa. Tale contributo è ben visibile anche osservando i valori riportati in tabella. Si fa presente che tali contributi apportino un incremento della potenza dinamica dissipata.
\\
Una situazione duale si presenta in corrispondenza di un fronte di discesa dell'uscita.
\\\\
...
\\\\
Prima di avviare la simulazione sono state inserite le seguenti direttive al fine di ottenere dati relativi all'andamento delle correnti.
\begin{center}
\begin{listato}
	\centerline{\lstinputlisting{./code/Lab_5/direttive_correnti.txt}}
\end{listato}
\end{center}
\vspace{0.3cm}
Successivamente sono state analizzate le forme d'onda delle tensioni in uscita alle due porte rispetto alla transizione in ingresso.
\\\\
...
\end{center}
\vspace{0.3cm}
È possibile notare come la porta di dimensioni maggiori sia associata a consumi di potenza maggiori. Si osserva inoltre l'indipendenza della potenza dissipata rispetto al carico utilizzato in uscita. \textcolor{red}{spiegare il motivo}
\\\\
...
\\
In particolare, tali valori presentano un incremento rispetto alla NAND standard. Ciò è dovuto all'aumento delle dimensioni dei transistor.
...
\\\\
Osservando le forme d'onda, si nota come l'andamento sia analogo al caso high speed. Le differenze rispetto al caso precedente sono da ricercare nell'ampiezza dei picchi relativi alla corrente di corto circuito.
\\\\
\begin{center}
	\begin{tabular}{|c|c|c||c|c|}
	\hline
...
\end{center}
\vspace{0.3cm}
In generale, vi è un incremento dei ritardi rispetto alla porta ottimizzata per high speed. Rimane circa invariato il rapporto di aumento di ritardo dovuto all'incremento di capacità di carico.
\\\\
\begin{center}
	\begin{tabular}{|c|c|c||c|c|}
	\hline
...
\end{center}
\vspace{0.3cm}
Come atteso, i picchi di corrente nel caso della porta low leakage risultano essere di valore inferiore rispetto al caso high speed.
\\
...
\end{center}
\vspace{0.3cm}
\textcolor{red}{commento da inserire}
\\\\
\begin{center}
	\begin{tabular}{|c|c|}
	\hline
